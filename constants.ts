...
export const FORMATIVE_LEVEL_INFO: Record<number, { label: string; score: number; predicate: Exclude<Predicate, ''>; color: string }> = {
    4: { label: 'Mahir', score: 95, predicate: 'A', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' },
    3: { label: 'Cakap', score: 85, predicate: 'B', color: 'bg-indigo-100 text-indigo-800 border-indigo-300' },
    2: { label: 'Layak', score: 75, predicate: 'C', color: 'bg-amber-100 text-amber-800 border-amber-300' },
...
export const SCORE_PREDICATE_MAP: { minScore: number; info: ScorePredicateInfo }[] = [
  { minScore: 90, info: { predicate: 'A', label: 'Mahir', level: 4, color: 'bg-emerald-100', textColor: 'text-emerald-800' } },
  { minScore: 80, info: { predicate: 'B', label: 'Cakap', level: 3, color: 'bg-indigo-100', textColor: 'text-indigo-800' } },
  { minScore: 70, info: { predicate: 'C', label: 'Layak', level: 2, color: 'bg-amber-100', textColor: 'text-amber-800' } },
